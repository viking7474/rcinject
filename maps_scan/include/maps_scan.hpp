...
namespace maps_scan {
    ...
    struct SMapInfo;
    using SCallback = std::function<bool(const SMapInfo &)>;
    using SFilter = SCallback;
    struct SMapInfo {
        /// \brief The start address of the memory region.
        uintptr_t start;
        /// \brief The end address of the memory region.
        ...
        static std::vector<SMapInfo> Scan(std::string_view pid = "self", std::optional<const SFilter> filter = std::nullopt);

        static void ForEach(const SCallback &callback, std::string_view pid = "self");

        static inline std::vector<SMapInfo> ScanSelf(std::optional<const SFilter> filter = std::nullopt) {
            return Scan("self", filter);
        }
        ...
        inline std::string PermStr() const {
            std::string res;
            res += (perms & PROT_READ) ? 'r' : '-';
            res += (perms & PROT_WRITE) ? 'w' : '-';
            res += (perms & PROT_EXEC) ? 'x' : '-';
            res += is_private ? 'p' : 's';
            return res;
        }
        std::string display() const;
