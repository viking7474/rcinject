...
namespace {
    ...
    std::tuple<uintptr_t, size_t> OpenLibrary(const std::string_view path) {
        ...
        if (fd < 0) {
            PLOGE("elf_parser: open {}", path.data());
            return {0, 0};
        }

        struct stat st{};
        if (fstat(fd, &st) < 0) {
            PLOGE("elf_parser: stat {}", path.data());
            close(fd);
            return {0, 0};
        }
...
namespace elf_parser {
    ...
    void Elf::MayInitLinearMap(bool dyn) const {
        // In some Elfs, dyn syms don't appear again in their symtab ...
        if (dyn && !dyn_added_) [[unlikely]] {
            if (dynsym_ && dynstr_) {
                if (gnu_bucket_ && gnu_chain_) {
                    for (uint32_t i = 0; i < gnu_nbucket_; i++) {
                        auto idx = gnu_bucket_[i];
                        if (idx == 0) continue;
                        for (;;) {
                            auto hash = gnu_chain_[idx];
                            auto &sym = dynsym_[idx];
                            unsigned int st_type = ELF_ST_TYPE(sym.st_info);
                            const char *st_name = dynstr_ + sym.st_name;
    ...
    SymInfo Elf::getSymByAddr(uintptr_t addr) const {
        SymInfo info{};
        forEachSymbols([&](const char* name, ElfW(Sym) *sym) -> bool {
            auto start = sym->st_value + GetLoadBias();
            auto end = start + (sym->st_size == 0 ? 1 : sym->st_size);
